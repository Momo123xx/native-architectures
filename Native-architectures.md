...
- **Bug-fix release. No new theoretical content. One observational correction (§9.1).**
- **§4.3 scout rigid-prior prediction scoped to pure scouts**: Added AuDHD caveat noting that AuDHD scouts show a different stress cascade — both tiers amplify (scout acceleration + systemizer co-activation) while learned inter-tier integration degrades. Cross-referenced to updated §9.1.
- **§9.1 stress-polarization self-report corrected**: Previous characterization ("reversion to logic-tree construction (systemizer)") replaced with more accurate phenomenological description — primary amplification is scout (accelerated angle-seeking), systemizer co-activates at degraded effectiveness (logic chains loop, working memory floods), learned inter-tier integration sheds (nuance collapses, emotional loading overrides cognitive perspective-taking). §9.8 updated for consistency.
- **§4.11 engagement modes integrated with prior-formation channel language**: Systemizer, scout, and socialite engagement mode descriptions updated to reference their respective prior-formation channels. Missed in v1.7.1 integration pass.
- **§4.13.1 Dodson citation clarified**: Note added that rejection sensitive dysphoria construct originates in clinical practice literature and lacks peer-reviewed validation as a distinct mechanism.
- **§9.3 error-signal phrasing clarified**: "without equivalent error signals" → "without error signals of equivalent type or salience" to resolve surface-level contradiction with bidirectional disconnection described in same section.
- **§4.5 terminological fix**: "locked" → "fixed-policy" where referring to non-negotiable precision policies, to avoid collision with §2.1's technical definition of "locked" (model-completeness endpoint state).
- **§4.14 opening softened**: "must be revised through those same channels" → "are most effectively revised through those same channels," consistent with the differential-threshold mechanism the section actually describes.
- **§5.2 cross-reference added**: §4.12 (masking vs. deliberate development) added to internal tension note.
- **§4.5 gradient scope note added**: Acknowledges that gradient positions exist on all three edges of the triadic space; AuDHD maps to the scout-systemizer edge.
- **§3.2 loop restart detection specified**: Added architecture-mediated detection mechanism for the cohere→explore transition.
- **§2.1 defensive sequence support scoped**: §9.11 consistency claim clarified to note it evidences mid-sequence behavior, not the full four-step trajectory.

### v1.7.1 (from v1.7)

- **Bug-fix release. No new theoretical content.**
- **§1 updated**: Core framework description now leads with prior-formation channel as primary differentiator, consistent with v1.7 core revision. Previous "three distinct default precision weighting policies" framing demoted to consequence.
- **Prior-formation channel disambiguation (§2)**: Terminological note added distinguishing "prior-formation channel" (type of evidence trusted for prior-building) from "channel" (processing pathway/information stream) to prevent conflation.
- **§4.6 monotropism integrated with prior-formation channel language**: Systemizer, scout, and AuDHD monotropism descriptions updated to reference their respective prior-formation channels. AuDHD entry cross-referenced to §4.5 hierarchical precision split.
- **§4.12 growth trajectories integrated with prior-formation channel language**: Systemizer → scout and scout → systemizer development paths now reference the specific prior-formation channel mechanisms rather than generic "lower/raise prior precision."
- **§4.3 scout rigid-prior prediction softened**: Changed from "confirmed by" to "consistent with" theorist's self-report. Added n=1 limitation, RSD confound, and note that serialization-under-stress may vary across scouts.
- **§3.2 translation dependency conditionality flagged**: Added note acknowledging the error-corrective function depends on the translation mechanism (§4.8), which is itself an untested prediction. Translation quality varies continuously; some architecture pairings may be too distant for effective error-correction even with high-quality translation.
- **§6.3 AuDHD interaction dynamics formally closed**: Antagonistic masking, resource competition, and paradoxical motivation were removed in pre-v1.3 versions and fully superseded by the gradient model (§4.5).
- **Appendix A corrected**: Two "refined v1.8" references corrected to v1.7. Shared-datum annotations added for §9.1/§9.8.
- **Appendix B item 4 updated**: Release recommendation now points to current version rather than v1.5.

### v1.7 (from v1.6)

- **Core revision: Channel of prior formation as primary differentiator.** The three architectures differ not in their capacity for high or low priors, but in the channel through which priors are primarily built, maintained, and revised. Systemizer: internal model coherence. Scout: experiential investment. Socialite: social validation. Existing precision-level descriptions (high/volatile/flexible) are preserved as descriptions of typical operating state; the channel revision explains why those states arise and when they don't hold. All architecture definitions (§2.1–§2.3) and structural properties (§3.1) updated accordingly.
...
- **Scout precision profile refined**: Prior precision recharacterized as volatile (low default set point, capable of temporary elevation through investment, reverting under load) rather than uniformly low. PE salience as consequence of low default is preserved. All downstream references updated for consistency.
- **New derived mechanism**: Open loop stress cascades (§4.13) — when stress-polarization locks the system into a native strategy that cannot resolve the specific stressor type, the PE persists without resolution. Defensive priors form around avoidance of open-loop-producing conditions.
- **Architecture-specific open loop expressions**: Scout (§4.13.1) — anticipatory threat-scanning with feedback risk. Systemizer (§4.13.2) — variable elimination targeting high-entropy inputs. Socialite (§4.13.3) — placeholder, predicted but unconstrained by observation.
- **New observational evidence**: §9.12 — scout open loop and anticipatory scanning (theorist self-report, [MOTIVATING], developmental confound acknowledged).
- **No existing theoretical content revised**: All v1.5 mechanisms, predictions, and structural claims carried unchanged. Volatile prior precision is a refinement of §2.2, not a replacement of the scout architecture's position in the triadic space.
...
The three architectures are unified under this framework as **three distinct prior-formation channels** operating on the same underlying predictive processing machinery — differing not in their capacity for high or low priors, but in the channel through which priors are primarily built, maintained, and revised (§2). The resulting precision-weighting policies (high/volatile/flexible) describe the typical operating state of each architecture; the prior-formation channel explains why those states arise and when they don't hold. Same engine, different input channels to the gain dial.
...
**Prior-formation channel**: The three architectures differ not in their capacity for high or low priors, but in **the prior-formation channel through which priors are primarily built, maintained, and revised**. The level of prior precision at any given time is a consequence of the prior-formation channel's state, not a permanent tuning parameter. All three architectures start with low priors on new domains, build priors through their native prior-formation channel, and are capable of rigidity when the channel's conditions are met. What differs is the prior-formation channel itself — the type of evidence the system treats as prior-building.

**Terminological note**: "Prior-formation channel" refers specifically to the type of evidence an architecture trusts for building, maintaining, and revising priors (model coherence, experiential investment, or social validation). This is distinct from "channel" used elsewhere in this document to denote a processing pathway or information stream (e.g., "social channel," "sensory PE channel," "competing channels"). The prior-formation channel determines *how* priors are earned; processing channels are *where* precision is allocated.

| | Systemizer | Scout | Socialite |
|---|---|---|---|
...
This revision does not replace the PP precision-weighting framework — it specifies **where in the system** precision is allocated, not just how much. The existing precision-level descriptions (high/volatile/flexible) remain accurate as descriptions of the *typical operating state* of each architecture. The channel-of-prior-formation revision explains *why* those typical states arise and *when they don't hold*. The PP machinery is preserved: precision weighting is still the gain-control mechanism. The refinement is that the gain is modulated by different evidence sources depending on the architecture. Same engine, different input channels to the gain dial.
...
**Precision profile**: High sensory PE precision + model-completeness-dependent structural prior precision + inflexible reallocation.

**Prior-building channel**: Internal model coherence. The systemizer builds priors through logical structure and causal consistency. Prior precision is not a permanent high setting — it is **model-completeness-dependent**: low on nascent domains (high gain, absorptive mode), monotonically rising with model-building, locking once the model achieves internal coherence. The "high structural prior precision" description captures the locked state — the endpoint of the trajectory, not a permanent tuning parameter.

| Model state | Prior precision | System behavior |
|---|---|---|
| Nascent (new domain) | Low | High gain — absorbing, consolidating, building |
| Developing | Rising | Selective gain — integrating compatible input, flagging incompatible |
| Locked (internally coherent) | High, inflexible | Low gain on that domain — incoming PEs dismissed, reinterpreted, or resisted |

In the typical operating state — the systemizer engaged with domains where models have been built — prior precision presents as high and inflexible. This is the state most frequently observed and the state the existing literature describes. The model-completeness trajectory explains why systemizers can be voraciously absorptive in new domains while being rigidly resistant in established ones — the same architecture, at different points in the prior-building trajectory.

The system trusts incoming sensory detail (bottom-up PEs weighted high) and trusts its own built models (structural priors weighted high once locked). This produces the characteristic tension: rigorous models AND sensory flooding. Monotropic expression — deep, narrow, persistent focus — is a consequence: concentrating precision on a single channel manages saturation, and high prior precision on the locked model resists channel-switching.

**Defensive sequence (locked model)**: When the systemizer's model is locked and incoming evidence contradicts it, the framework predicts a specific sequence of defensive responses before revision occurs: (1) Dismiss as noise — PE suppressed by high prior precision. (2) Reinterpret to fit — PE absorbed into existing model structure. (3) Defensive escalation — if evidence persists, emotional loading increases (the frustration pattern described in §9.11). (4) Forced revision — only when accumulated contradictions exceed the model's structural capacity to absorb them. This sequence is consistent with the §9.11 observation — which evidences the mid-sequence stages (frustration and model-defense) but has not been observed as a complete four-step trajectory — and with the general PP prediction that high-confidence priors resist updating proportionally to their precision.

**Intrinsic cost** (rigidity): Once locked, high prior precision on structural models means the threshold for PE-driven model revision is very high. The model resists updating because it has assigned high confidence to itself. Revision requires not just counter-evidence but accumulated logical contradictions sufficient to dismantle the model's internal coherence — the systemizer's native crack point.
...
The "scout" framing is preferred over Hartmann's (1993) "hunter" metaphor because it maps directly onto the PP mechanism: exploration of novel territory under volatile prior precision (low default), rather than a predator-prey analogy that does not connect to the underlying computational architecture.

**Precision profile**: Volatile prior precision (low default set point) + rapid reallocation.

**Prior-building channel**: Experiential investment. The scout builds priors not through logical model coherence (systemizer channel) or social validation (socialite channel) but through personal cost — the accumulated investment of exploratory effort, time, and emotional commitment. Where the scout has paid the price of deep engagement, prior precision rises. The investment cost is the anchor, and the crack point is a novel signal salient enough to outbid the sunk investment.

Prior precision is volatile — low by default on unsystemized domains, capable of temporary elevation through sustained investment, but unable to maintain elevated precision with the stability the systemizer achieves. The low default means prediction errors from novel or unexpected input get amplified disproportionately because the system typically lacks high-confidence priors to suppress them. Where the scout has built a model through deliberate effort, prior precision on that domain rises — but remains more fragile than the systemizer's equivalent, reverting toward the low default under load (consistent with stress-polarization, §4.3). Novelty is intrinsically salient because the prior is typically too weak to explain away the PE. High PE salience in the scout is a *consequence* of the low default prior precision, not a separate tuning parameter — when priors are weak, more prediction errors pass the significance threshold, making the world feel continuously novel. During periods of elevated prior precision (e.g., hyperfocus on an invested domain), the temporary prior elevation raises the threshold for competing channels to outbid the current one — functioning as momentum rather than suppression. Unlike the systemizer's locked priors, which reach suppressive strength and genuinely gate incoming PEs, the scout's elevation remains permeable to sufficiently salient external signals throughout (see §4.11 for the engagement mode implications). The elevation is fragile and reverts under load. **Hierarchical note**: In the pure scout profile, volatile prior precision with low default operates globally — across both within-model and meta-model levels of the hierarchy. This distinction becomes load-bearing in the AuDHD case (§4.5), where prior precision splits across hierarchical levels.

**Rigid-prior case**: The channel-of-prior-formation revision resolves an anomaly in the prior description: the scout holding a rigid prior. The volatile-prior description captures the default, not the full range. A scout who has painstakingly developed a systematized channel through enormous personal investment can hold rigid priors on that channel — not because of logical coherence (systemizer mechanism) but because of investment cost. Different anchor, different crack point, same behavioral output (rigidity). Under stress, the systemizer-mode maintenance of the channel (careful logical upkeep) sheds — this is non-native infrastructure. The scout-native investment anchor (emotional weight of the personal cost) holds. The scout under stress holds the *commitment* to the channel but loses the *precision* on it — defends it instinctually, cannot articulate the defense logically (see §4.3 stress-polarization shedding order).

**Intrinsic cost** (commitment difficulty): Volatile prior precision means the system cannot maintain high confidence in its current model long enough to resist the next salient PE. Even when prior precision temporarily elevates through investment, it lacks the stability to hold against competing PEs indefinitely. The current engagement is always vulnerable to being outbid by incoming surprise. The same volatile prior precision that makes the scout brilliant at finding new territory makes it difficult to stay once it has been found.
...
**Prior-building channel**: Social validation. The socialite builds priors through consensus, group agreement, and reciprocal confirmation. The socialite's default response to a new domain is to check what others think, weight consensus heavily, and build priors through social confirmation rather than through internal model-building (systemizer channel) or exploratory experience (scout channel). Prior precision on a domain rises with social consensus and stabilizes when group norms are established. The crack point is consensus shift — holding the old position means standing alone.

The system assigns high default precision to the social prediction channel — violations of expected social behavior, breaks in reciprocity, failures of group coordination are weighted more heavily than non-social PEs. The system treats social signals as its most reliable information source. Across all other channels, precision allocation is context-sensitive and flexible — the socialite can shift between prior-dominant and PE-dominant processing depending on situational demands. The flexibility is not unconstrained: because the socialite's prior-building channel is social validation (§2), non-social priors also tend to track group consensus — what the group believes about a domain informs what the socialite believes about it. This consensus-tracking across domains is the mechanism of the flexibility — adaptive because it allows rapid updating when social context shifts, constrained because it depends on social input even for non-social questions. Consensus-tracking is not exclusive to the socialite — all architectures can use social input to build priors, especially through learned cross-architecture development (§4.12) — but it is the socialite's native, automatic channel, the one that runs without volitional cost.
...
The PP mapping reveals that the three architectures are not structurally identical types of entity. All three are dynamic systems — precision levels change with development, context, and model-state — but they differ in what is held fixed:

The **systemizer** profile is a fixed-position dynamic — prior precision tracks model-completeness (§2.1), locking high once internally coherent. The fixedness is in the endpoint and the trajectory, not in a static setting.

The **scout** profile is a fixed-dynamic — a volatile precision policy (low default, temporarily elevatable through investment) that produces a characteristic pattern of rapid reallocation. The fixedness is in the policy itself: the system cannot stop being volatile.

The **socialite** profile is a fixed-anchor strategy — one channel (social) committed at high precision, with flexible context-sensitive allocation across all other channels. The fixedness is in the anchor; the flexibility is in everything else.

All three share one structural feature: a non-negotiable precision commitment. For the systemizer, this is a high-precision ceiling — sensory-PE precision locked high, structural-prior precision rising with model-completeness and locking once internally coherent (§2.1). For the socialite, it is a high-precision anchor — the social channel is fixed high. For the scout, it is a volatile-precision floor — prior precision defaults low and is capable of temporary elevation through investment but unable to maintain stability, which makes novelty salient as a consequence of the low default (see §2.2), not as a separately assigned high-precision channel. They differ in what they do with the remaining precision budget. Systemizers lock it onto sensory detail. Scouts cannot prevent it from following the next unsuppressed PE. Socialites distribute it flexibly based on context.

The architectures are **functionally symmetric** (each is necessary, each has costs, each is a peer in the loop) but **structurally asymmetric** (two have fixed precision policies — one positional, one dynamic — while the third is a strategy over the precision space, anchored by one fixed channel). The socialite is not "better" — flexibility is a different optimization, not a superior one. The fixed-policy profiles access regions of the precision space that flexibility cannot reach by definition. Depth requires a fixed policy.
...
The channel-of-prior-formation revision (§2) adds a dimension to this structural account. The three architectures differ not only in their precision configuration but in the channel through which precision is earned. The systemizer's locked high priors are locked because they were built through internal model coherence — they are high because the model is complete, and they resist revision because revision requires dismantling logical dependencies. The scout's volatile low-default priors are volatile because they were built through experiential investment — temporary, fragile, and anchored by personal cost rather than logical structure. The socialite's flexible allocation is socially anchored because precision is earned through social validation — the socialite's priors on any domain track the group's consensus on that domain. The structural asymmetry is preserved: two architectures have fixed policies operating through fixed channels (model coherence, experiential investment), one has a flexible allocation anchored by a fixed channel commitment (social validation). The channel revision explains *why* each architecture's precision configuration is stable — it is held in place by the type of evidence the architecture trusts.
...
Each architecture's output is the next architecture's required input. Scouts generate signal from unknown territory. Systemizers extract structure from that signal. Socialites cohere groups around established structure. The cycle restarts when established structure encounters novel territory — the scout's native PE salience positions them to detect environmental change that exceeds the current structure's predictive capacity, making the restart itself architecture-mediated.
...
**Translation dependency**: The error-correction function is not automatic — it requires the detecting architecture's signal to reach the architecture running the excess. The trust-channel mechanism (§4.14) predicts this signal will be rejected if delivered in the detector's native format rather than the receiver's. Error-correction therefore depends on architectural translation (§4.8): either a dedicated translator, or — more commonly — mutual translation where both parties approximate the other's trust channel. No 1:1 communication protocol between architectures exists; translation is always lossy, always approximate, and always effortful. Without translation capacity, the error-correction function degrades and each architecture's excess goes unchecked. The loop's dual-function claim (generative + error-corrective) is therefore conditional: the generative function operates regardless, but the error-corrective function scales with the translation capacity available in the system.

**Conditionality note**: The error-corrective function depends on translation capacity (§4.8), which is itself a prediction of this framework, not an established mechanism. The dependency is therefore doubly conditional: it requires translation to be structurally possible (the architectural claim) and translation to be empirically present in a given system (the environmental claim). Translation quality varies continuously — the better A is at matching B's trust channel, the more error-correction bandwidth exists. But some architecture pairings may be sufficiently distant that even high-quality translation cannot establish a functional information link. The error-corrective function is a capacity of the loop, not a guarantee.
...
**AuDHD scout caveat**: For AuDHD scouts, the within-model systemizer component (§4.5) co-amplifies under stress rather than shedding. The theorist's self-report (n=1, with significant rejection sensitivity as a confound — see §4.13.1; see §9.1 for full phenomenological description) shows both scout acceleration (increasingly desperate angle-seeking) and systemizer logic-chain formation amplifying simultaneously, while learned inter-tier integration degrades — nuance collapses, working memory floods, logic chains loop back on each other, and emotional loading (felt unfairness) overrides cognitively-held perspective-taking. The self-report is consistent with the meta-level prediction (commitment holds, scouting amplifies) but shows within-model amplification rather than precision loss. This is consistent with stress-polarization applied tier-by-tier (§9.1 AuDHD hierarchical note): each hierarchical level reverts to its own native setting, and the learned coordination between tiers is itself a capacity that sheds under load.
...
**Categorical dependency note**: This formulation — orbits around attractors — presupposes that the attractor basins exist as discrete entities. As §3.3 acknowledges, the categorical vs. dimensional question is empirically open. If the space proves to be dimensional rather than categorical, plasticity would need to be reformulated as movement along a continuum rather than orbit modification around a fixed point. The mechanism described here is conditional on categoricality.

### 4.5 AuDHD as Attractor Gradient

AuDHD is not "autism plus ADHD" as two discrete modules bolted together. It is a position in the precision-weighting space between the scout and systemizer attractors. Both profiles share the trait of a fixed precision policy (unlike the socialite's flexibility), but fixed in different directions — the scout cannot stop exploring (fixed volatile-low-default), the systemizer cannot stop structuring (prior precision locking high once models are coherent, per §2.1). "Fixed" refers to the precision *policy*, not to the behavioral output, which differs dramatically between the two.

**Hierarchical precision split**: The apparent contradiction — how can the AuDHD individual have both high structural priors (systemizer) and high PE salience from low priors (scout) simultaneously? — resolves when the precision policies are located at different levels of the predictive hierarchy. The systemizer component operates **within-model**: high prior precision on the structure being built, high confidence in the internal model's coherence. The scout component operates **on-model**: volatile prior precision (low default) on whether the model itself is valid, whether the entire enterprise should exist, whether the current direction is correct. These are not competing claims about the same channel — they are different precision policies at different hierarchical levels, running concurrently.

This produces AuDHD's characteristic signature: intense model-building (systemizer within-model priors driving depth and coherence) combined with catastrophic fragility (scout meta-level priors failing to suppress prediction errors about the model's validity). A minor external critique or internal inconsistency generates a PE at the meta level. Because the scout's volatile meta-prior (low default) cannot absorb it, the PE propagates. Because the systemizer has built a tightly-coupled model where structural coherence is high, a single meta-level crack threatens cascading collapse — the system cannot localize the damage because everything depends on everything. The phenomenological result is disproportionate to the trigger: a small signal produces a sense that the entire model is wrong and the entire effort was wasted.
...
**Contrast with pure profiles**: A pure systemizer, once models are locked, has high priors at *both* levels — within-model and on-model-validity — so minor critiques bounce off the meta-prior. The model only collapses when accumulated external evidence overwhelms the high meta-prior, which takes substantial and sustained pressure. A pure scout has volatile priors (low default) at *both* levels — models are held loosely throughout, so there is no tightly-coupled structure to collapse and no catastrophic fragility. The AuDHD combination — high within-model, volatile-low on-model — is what produces the unique phenomenology: the flywheel (intense coherent building) and the crash (disproportionate collapse from minor signals).

Each AuDHD individual sits somewhere on the gradient between the two fixed-policy profiles, with a dominant pull toward one.

**Gradient-position-dependent hierarchy**: The hierarchical precision split described above reflects the **scout-dominant** position on the AuDHD gradient: high within-model precision (systemizer component) + volatile on-model-validity precision (scout component), producing the flywheel-and-crash phenomenology. A systemizer-dominant AuDHD individual is predicted to experience the inverse:

| Gradient position | Within-model precision | On-model-validity precision | Phenomenology |
|---|---|---|---|
| Scout-dominant (described above) | High (systemizer component) | Volatile, low default (scout component) | Intense building → catastrophic meta-doubt ("is this even worth doing?") |
| Systemizer-dominant (predicted) | Volatile (scout component disrupts construction) | High (systemizer component: the model IS right) | Knows exactly what to build → keeps getting pulled off the construction path by novel within-model PEs |

**[UNDERCONSTRAINED]** — The scout-dominant description is grounded in the theorist's self-report (n=1 with deep introspective access). The systemizer-dominant prediction is structurally derived from inverting the hierarchical split but has no observational data. Confirming this prediction requires self-report from introspective systemizer-dominant AuDHD individuals capable of distinguishing within-model from meta-level precision experiences.

**Channel-of-prior-formation integration**: The hierarchical precision split interacts with the channel revision (§2). In scout-dominant AuDHD, the within-model tier builds priors through the systemizer's native channel (internal model coherence — the model holds because it is logically consistent), while the meta tier builds priors through the scout's native channel (experiential investment — the model is worth pursuing because of what it cost to build). The channels at each tier match the precision policy at that tier. This assignment is predicted to invert with gradient position: systemizer-dominant AuDHD should build meta-level priors through model coherence (the model IS structurally right) and within-model priors should be disrupted by the scout channel (experiential PEs pulling attention from the construction path). **[UNDERCONSTRAINED]** — the channel-at-tier assignment is derived from one scout-dominant case; confirmation from systemizer-dominant AuDHD self-report is needed.

The general claim — that AuDHD involves a hierarchical precision split where the two components operate at different levels — is preserved. The specific assignment of which component operates at which level is gradient-position-dependent.

**Gradient scope note**: The AuDHD gradient maps to the scout-systemizer edge of the triadic space. Gradient positions also exist on the other two edges (socialite-scout, socialite-systemizer), each with its own predicted hierarchical precision split and phenomenology. These are not addressed in this version — AuDHD is the clinically labeled case and the only edge with observational data. The framework predicts all three edges are populated; the other two await characterization.
...
- **Systemizer monotropism**: Deep, narrow, persistent. Prior precision built through the model-coherence prior-formation channel (§2.1) resists being outbid by competing PEs — the model is internally coherent, and switching means dismantling logical dependencies, which the system experiences as structural damage. Channel holds for years or a lifetime because the coherence anchor does not decay with time.
- **Scout monotropism**: Deep, narrow, temporary. When engaged, intensity can match the systemizer's — but the prior elevation is built through the experiential-investment prior-formation channel (§2.2), not through model coherence. The channel holds as long as the accumulated investment cost anchors it; the moment a novel signal outbids the sunk investment, the prior reverts toward its low default and a new channel takes over. The temporary prior elevation during intense engagement produces systemizer-like depth, but the investment anchor is fragile compared to the coherence anchor.
- **AuDHD monotropism**: Deep, narrow, medium-duration. Systemizer component drives depth (within-model coherence anchor); scout component drives rotation (meta-level investment anchor). Position on the AuDHD gradient (§4.5) determines the ratio — scout-dominant AuDHD should show shorter monotropic episodes with more catastrophic exits; systemizer-dominant should show longer episodes with more resistance to novel within-model PEs.
...
**Upstream mechanism**: The trust-channel mechanism (§4.14) explains why protocol-switching is necessary and what specifically must change. Protocol-switching is the behavioral adaptation; trust-channel matching is the evidential requirement. Effective cross-architecture communication requires both — translating the format (protocol-switching) and matching the evidence type (trust-channel).
...
**Systemizer native engagement mode**: All available precision budget concentrated on the structural-prior channel and its associated sensory inputs. Competing channels suppressed to near-zero. The high prior precision on the model being built locks the channel in place; the high sensory PE precision on relevant detail creates total immersion in the subject. Entry condition: deep subject-alignment — the system must be engaging with a domain where the model-coherence prior-formation channel (§2.1) has built or is building a high-confidence model. This explains why systemizers can sustain intense focus for hours on their domain of engagement but cannot force the same state on an unrelated task — the prior precision is not present to anchor the channel. This state more closely resembles hyperfocus as described in the clinical literature — intense sustained attention with diminished perception of non-task stimuli — than formal flow. Research shows hyperfocus and flow are related but distinguishable: hyperfocus is associated with perceived loss of control and can override self-care, while flow involves a sense of effortless control (Hupfeld et al., 2019; Ashinoff & Abu-Akel, 2021).

**Scout native engagement mode**: The effective PE salience — a consequence of volatile prior precision at its low default (§2.2) — is continuously sustained because the unfolding activity itself generates a stream of novel prediction errors. Each moment of execution generates enough prediction error — micro-novelty, micro-discovery — to sustain engagement. The volatile prior precision means the scout is not locked onto a model but is riding the PE stream in real time. During this state, the PE stream itself temporarily elevates prior precision on the active channel through the experiential-investment prior-formation channel (§2.2) — each moment of engagement is an investment, and the engagement sustains its own priors — but this elevation operates as momentum, not suppression. Unlike the systemizer's locked priors, which reach suppressive strength and genuinely gate incoming PEs, the scout's temporary elevation merely raises the bar for competing channels to outbid the current one. The system remains permeable to sufficiently salient external PEs throughout — a sudden noise, a novel external signal, a notification can break through at any point because the elevated prior never reaches the suppressive strength of a systemizer's locked model. The engagement is a flywheel: sustained by continuous novelty input from the activity, decelerating when the input drops, and interruptible by sufficiently salient external signals at any point. This elevation is fragile and collapses when the novelty stream drops. Process and exploration merge: the activity itself becomes the adventure. Entry condition: process-engagement where the activity generates continuous novelty. This explains why scouts enter this state during improvisation, real-time problem-solving, or action-oriented tasks, but struggle with repetitive execution where the PE stream dries up. The scout's engagement collapses when novelty drops, even if challenge-skill balance is maintained — which standard flow theory does not predict.

**Socialite native engagement mode**: Social-channel precision at maximum, with social prediction errors being resolved in real time through reciprocal exchange. The flexible non-social precision budget is freed because the social anchor is fully satisfied — the protocol handshake is not just complete but running at high bandwidth. This reduces the precision budget normally consumed by unresolved social PEs — the social channel still runs, but at equilibrium rather than in deficit. Entry condition: genuine reciprocal social engagement — the social-validation prior-formation channel (§2.3) must be receiving high-quality, high-bandwidth signal through authentic emotional exchange, not performative socializing. This explains why socialites can feel deeply connected in intimate conversation or collective crisis but drained by shallow networking — the former saturates the social channel, the latter generates unresolved social PEs. This state more closely resembles group flow or shared interactive flow (Sawyer, 2003; Hackert et al., 2023) — collective synchrony, self-other overlap, seamless coordination. The triadic model predicts socialites access this state with lower thresholds and higher frequency than other architectures.
...
**Systemizer → Scout development**: The systemizer's depth becomes limiting when the individual has been building deeply in territory that is no longer relevant, or has never explored beyond the current domain. The model-coherence prior-formation channel (§2.1) has driven prior precision to the lock point, and the locked model now prevents release and territory-seeking. Growth requires learning to release locked models before the coherence channel has driven them to completion — to tolerate model-incompleteness, to disengage from a logically consistent structure without resolving every dependency. The systemizer architecture experiences model-abandonment as structural damage (dismantling logical dependencies), which is why this is the path of most resistance.

**Scout → Systemizer development**: The scout's exploration becomes limiting when the individual has found many worthy subjects but built none of them to depth. The experiential-investment prior-formation channel (§2.2) produces fragile elevation that cannot sustain the deep model-building the systemizer achieves through coherence. Growth requires learning to build priors through model coherence rather than through accumulated personal cost — to stay with one thing because the logical structure rewards depth, not because the investment cost makes leaving painful. The scout architecture experiences sustained commitment as confinement, which is why this is the path of most resistance.
...
### 4.13 Open Loop Stress Cascades

Stress-polarization (§4.3) strips the system to its native resolution strategy. Under normal conditions, the native strategy resolves the stressor and the system returns to baseline. An **open loop** forms when the native strategy cannot terminate the stressor — the PE keeps firing, the system keeps running its native response at increasing intensity, but the error signal does not resolve.

**PP mechanism**: Stress-polarization depletes top-down control resources, leaving only the native precision-weighting policy. If the stressor type is one that the native policy cannot address — a social rejection PE hitting a scout who cannot explore away from it, a structurally irrational stressor hitting a systemizer who cannot decompose it — the system is locked into a response mode that cannot produce resolution. The PE remains active. The native response amplifies. Metabolic cost accumulates without error reduction.

**Defensive prior formation**: Repeated or sustained open loops produce a learned meta-prior: the system encodes avoidance of conditions identified as open-loop-producing. This defensive prior is architecture-specific in expression because it is built by the native precision-weighting policy operating on its own failure history.

The open loop mechanism is a derived consequence of stress-polarization, not an independent claim. If stress-polarization holds (§5.1), open loops follow for any case where the native strategy is type-mismatched with the stressor.

**Scope limitation**: Defensive prior formation is proposed as a mechanism. The specific developmental and environmental conditions that produce chronic open loops — and the interaction between architecture and developmental history in determining defensive prior severity — are acknowledged as important but are not formalized in this version. The mechanism describes *what* the system does when the native strategy fails; it does not yet specify the boundary conditions for *when* this becomes pathological versus adaptive.

See §4.13.1 and §4.13.2 for architecture-specific expressions where observational or structural evidence constrains the description.

#### 4.13.1 Scout Open Loop Expression

The scout's native stress response is exploration — seek an exit, scan for alternatives, move. An open loop forms when exploration cannot resolve the stressor: the scout is trapped in a situation that does not yield to scouting.

The characteristic defensive prior: anticipatory threat-scanning. The system learns to scout *before* commitment — scanning for conditions that previously produced unresolvable loops. The function is pre-emptive: detect open-loop-producing conditions and avoid them before the system becomes committed and unable to exit.
...
#### 4.13.2 Systemizer Open Loop Expression

The systemizer's native stress response is logical decomposition — identify the causal structure, isolate variables, eliminate the source. An open loop forms when the stressor is structurally irrational — it does not yield to logical decomposition because it has no stable causal structure the systemizer can model.

The characteristic defensive prior: variable elimination. The system identifies which input configurations produced the open loop and removes them from future engagement. The function is pre-emptive: eliminate variables that the model identifies as open-loop-producing before they can trigger a cascade.

**Degenerative trajectory**: High-entropy variables — those with the most unpredictable behavior — are eliminated first. People are the highest-entropy variables in most environments. The prediction: systemizer open loop cascades should produce social withdrawal *specifically* — not generalized withdrawal but targeted elimination of high-entropy social inputs. This is consistent with clinical descriptions of social withdrawal in autism, reframed here as downstream of variable elimination rather than as a primary social deficit.

**Evidential status**: The variable elimination mechanism is structurally derived — it follows from the open loop mechanism applied to the systemizer's high-prior precision profile. The specific prediction (people eliminated as highest-entropy variables) is illustrative — consistent with clinical patterns but not independently tested.

#### 4.13.3 Socialite Open Loop Expression

*[PLACEHOLDER]* The socialite's native stress response is social reconnection — mirror, affirm, re-establish the social protocol. An open loop forms when the social channel cannot be re-established — the other party is unavailable, unresponsive, or unable to reciprocate.

Observational data suggests: under stress, socialite mirroring intensifies, logical capacity degrades, attentional focus narrows to the social channel, and resolution occurs only through re-establishment of connection (not through logical or exploratory alternatives). This is consistent with stress-polarization to the social anchor.

Defensive prior formation is predicted (trust threshold escalation, protocol hardening) but not observationally constrained. The theorist's observational sample is ND-dominant and male-dominant, limiting visibility into chronic socialite open loops. This subsection requires socialite contributor input before defensive prior specification can move beyond prediction.

### 4.14 Architecture-Specific Trust Channels

If priors are built through architecture-specific channels (§2), they are most effectively revised through those same channels. Each architecture has a **trust channel** — the type of evidence it assigns high precision to when evaluating whether to revise an existing model.

| Architecture | Trust channel | What registers as valid evidence for revision |
|---|---|---|
| Systemizer | Logical demonstration | Structural contradiction, causal proof, internal inconsistency exposed |
| Scout | Direct experience / novel evidence | Encountering something that cannot be explained by the current model |
| Socialite | Social consensus shift | The group has moved; trusted others now disagree |

**Cross-architecture communication failure mechanism**: Cross-architecture communication fails not only because the content is misformatted (§4.9 protocol-switching) but because **the evidence for model revision is delivered through a channel the receiving architecture does not trust**.

- A systemizer presenting logical proof to a socialite delivers evidence through a channel the socialite assigns low trust-precision to. The socialite does not reject the logic because they cannot follow it — they reject it because logic alone, without social validation, does not meet their revision threshold.
- A socialite presenting social consensus ("everyone agrees you're wrong") to a systemizer delivers evidence through a channel the systemizer assigns near-zero trust-precision to. The systemizer does not reject the consensus because they are arrogant — they reject it because consensus, without structural logic, does not meet their revision threshold.
- A scout presenting experiential evidence ("I tried it and it works") to a systemizer delivers evidence through a channel the systemizer discounts without causal structure. The systemizer does not reject the experience because they distrust the scout — they reject it because anecdote without mechanism does not meet their revision threshold.

**Each interaction is rational from the receiver's trust channel.** No architecture is being stubborn. They are running different evidence-weighting policies on the incoming signal.

**Relationship to protocol-switching (§4.9)**: This mechanism is upstream of protocol-switching. Protocol-switching describes the *behavioral adaptation* — the communicator changes their output format. The trust-channel mechanism explains *why* the adaptation is necessary and *what specifically must change* — not just the format of the message but the *type of evidence* it contains. Effective cross-architecture communication requires delivering the revision through the receiver's trust channel, not just translating the vocabulary.

**Testable prediction**: See §5.10.
...
**Internal tension note**: This prediction sits in tension with the plasticity mechanism (§4.4), which says the orbit around the native attractor widens with development. If the orbit widens, one might expect the boundary to become more graded, not less. The proposed resolution: the phenomenological discontinuity persists regardless of orbit width — the cliff between automatic and override operation is always there, but an individual with extensive cross-architecture development can stand further from home before reaching it. The range of emulated operation widens; the nature of the boundary does not change. This resolution is plausible but untested. Note that §4.12's distinction between masking and deliberate development may affect this: masking conceals the boundary without moving it, while deliberate development may genuinely expand the pre-boundary range — if so, the two development paths make different predictions about where the discontinuity is experienced.
...
### 5.10 Trust Channel Matching

**Prediction**: An individual attempting to revise another's model should succeed at higher rates when the evidence type matches the receiver's native trust channel, independent of content quality. Logically sound arguments should fail more often with socialites (unless socially endorsed). Socially endorsed positions should fail more often with systemizers (unless logically grounded). Direct experiential demonstrations should fail more often with systemizers (unless accompanied by causal structure) and succeed more often with scouts.

**Differentiating against**:
- Triadic model predicts: Architecture-specific evidence-type sensitivity, with success rates tracking trust-channel match rather than argument quality alone.
- Content-quality model predicts: Better arguments succeed more regardless of audience architecture.
- Social influence model predicts: Social endorsement universally increases persuasion, regardless of receiver architecture.

**Status**: Derived from the channel-of-prior-formation revision (§2) and trust-channel mechanism (§4.14). Testable through persuasion studies that vary evidence type while controlling content quality across architecture-identified populations.
...
**AuDHD interaction dynamics — closed.** Original claims about antagonistic masking, resource competition, and paradoxical motivation (present in pre-v1.3 versions) have been fully superseded by the gradient reconceptualization (§4.5). Those claims were removed from the document body when the gradient model was introduced; this entry is now formally closed.

**Structural asymmetry unresolved.** Whether the structural asymmetry — two fixed-policy architectures and one fixed-anchor strategy — is a feature or a problem remains open.
...
**[STRUCTURAL]** Theorist self-report: Under argument stress, the theorist reports an accelerating cascade. The scout component amplifies first — uncontrollable, increasingly desperate scouting for more solutions and angles at increasing speed. The systemizer component co-activates — each angle generates rapid logic chains, but the chains begin looping back on each other as working memory floods. What degrades is learned inter-tier integration: positional nuance collapses (both the theorist's own position and the opponent's position lose granularity), the felt sense of unfairness overrides the cognitively-maintained perspective on the opponent's position, and the cascade produces somatic pressure requiring physical discharge. The learned Pareto capacity (accepting good-enough resolution rather than exhaustive coverage) sheds under load — both the scout's exploration constraint and the systemizer's completionism constraint give way. The structural feature — both native AuDHD components amplifying while learned cross-tier integration sheds — is the key datum, not the specific content.

**AuDHD hierarchical note**: The theorist is scout-dominant AuDHD. Under stress, both tiers amplify toward their native settings (§4.5): the scout component (meta-level) accelerates exploration, the systemizer component (within-model) accelerates logic-chain formation. What degrades is the learned integration between tiers — the capacity to constrain scout exploration with Pareto heuristics and to maintain nuanced perspective across competing logic chains. This is consistent with stress-polarization applied tier-by-tier — each level of the predictive hierarchy reverts to its own native setting — with the additional observation that inter-tier coordination is itself a learned capacity that sheds under load.
...
- Communication: The theorist's associative leaps occasionally produce errors in the brother when they violate structural expectations. The theorist tolerates the brother's structural rigidity without error signals of equivalent type or salience. The asymmetry in error type is consistent with different dominant attractors.
...
**[STRUCTURAL]** Theorist moved from rigid perfectionism to embracing Pareto heuristics through deliberate development. In PP terms: lowered prior precision on structural completeness and constrained scout exploration to accept good-enough resolution. However, under argument stress, both constraints shed — the native AuDHD stress cascade emerges (scout acceleration and systemizer logic-chain amplification; see §9.1 for full phenomenological description).
...
**Note**: This is the same data point as §9.1 (stress-polarization), here cited for the plasticity mechanism. The observation supports both mechanisms — the stress-reversion reveals both the native attractor (plasticity claim) and the shedding order (stress-polarization claim) — but constitutes a single observation doing double duty.
...
### 9.10 Cross-Architecture Communication Parsing (supports §4.9, §4.14, §4.1)
...
### 9.12 Scout Open Loop and Anticipatory Scanning (supports §4.13.1)

**[MOTIVATING]** Theorist self-report: The theorist reports interpreting neutral-to-positive social signals as negative-to-neutral, with automatic pre-emptive solution-generation triggered by ambient mood shifts. The theorist identifies this as a calibration artifact from a developmental environment where mood shifts reliably predicted threat (unpredictable caregiving, parentification). Cognitively recognized as miscalibrated; somatically persistent.

The structural feature: the anticipatory scanning operates pre-volitionally (consistent with §4.1 native vs. learned distinction) and produces false-positive threat detection (consistent with the §4.13.1 feedback mechanism). The theorist's ability to cognitively identify the miscalibration while being unable to suppress the somatic response is consistent with the native/override distinction — the scanning is automatic gain control, not top-down.

**Limitation**: Single-observer self-report. Developmental history (complex caregiving environment) is a confound — the pattern may be better explained by developmental trauma models (attachment disruption, CPTSD hypervigilance) than by architecture-specific mechanisms. The architecture-specificity claim requires testing the prediction that scouts with safe developmental histories still show elevated anticipatory scanning relative to other architectures with equivalently safe histories.
...
- Architecture-specificity of open loop products (§4.13) — the prediction that scouts show elevated anticipatory scanning and systemizers show targeted variable elimination *independent of developmental history* has not been tested. The §9.12 observation is motivating but confounded by developmental factors.
...
| Loop is dual-function: generative + error-corrective | PP-grounded; error-corrective function conditional on translation capacity (§3.2) | §3.2 | — | ↑ v1.3, refined v1.7 |
...
| Plasticity modifies range, not attractor | Confirmed by stress-reversion | §4.4 | §9.8 [STRUCTURAL] *(shared datum with §9.1)* | NEW v1.3 |
...
| Scout prior precision is volatile, not uniformly low | Refinement of §2.2 | PP-grounded: volatility preserves low-default while explaining temporary elevation; elevation operates as momentum/outbid-threshold, not suppression | — | NEW v1.6, refined v1.7 |
| Open loop stress cascades | Derived from §4.3 | PP-grounded: stress-polarization + type-mismatched stressor | §9.12 [MOTIVATING] | NEW v1.6 |
| Scout open loop → anticipatory scanning | Architecture-specific expression | §4.13.1 | §9.12 [MOTIVATING] | NEW v1.6 |
| Systemizer open loop → variable elimination | Architecture-specific expression | §4.13.2 | — | NEW v1.6 |
| Socialite open loop → trust threshold escalation | Predicted, not constrained | §4.13.3 [PLACEHOLDER] | — | NEW v1.6 |
| Channel of prior formation as primary differentiator | Core revision of §2, §3.1 | PP-grounded: specifies where precision is allocated, not just how much | — | NEW v1.7 |
| Systemizer prior precision tracks model-completeness | Architecture description revision | PP-grounded: a prior that hasn't been built can't be high | §9.11 [STRUCTURAL] | NEW v1.7 |
...
1. **Test architecture-specificity of rejection sensitivity.** The §4.13.1 prediction — that scouts show elevated anticipatory scanning independent of developmental history — is the critical test for whether the open loop mechanism adds explanatory power beyond trauma models. Cross-architecture comparison in populations with matched developmental safety would isolate the architecture contribution.
2. **Seek socialite contributors for §4.13.3.** The socialite open loop expression is predicted but unconstrained. Socialite self-report of chronic social-channel failure and resulting defensive patterns is needed before the placeholder can be replaced.
3. **Formalize open loop boundary conditions.** Specify when defensive prior formation becomes pathological versus adaptive. Without this boundary, the mechanism risks becoming a post-hoc absorber of developmental trauma phenomena.
...
4. **Open-source publication.** v1.5 was the first version with sufficient epistemic scaffolding — literature positioning, qualitative acknowledgment, confirmation bias disclosure, layer separation — for external contributors to engage productively. v1.7 extends that scaffolding with the prior-formation channel revision and trust-channel mechanism. Current version recommended for release.
...
Dodson, W. (2005). Pharmacotherapy of adult ADHD. *Journal of Clinical Psychology*, 61(5), 589–606. https://doi.org/10.1002/jclp.20122
