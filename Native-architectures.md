# Three-Architecture Neurodiversity Theory — v1.7
...
**Channel of prior formation**: The three architectures differ not in their capacity for high or low priors, but in **the channel through which priors are primarily built, maintained, and revised**. The level of prior precision at any given time is a consequence of the channel's state, not a permanent tuning parameter. All three architectures start with low priors on new domains, build priors through their native channel, and are capable of rigidity when the channel's conditions are met. What differs is the channel itself — the type of evidence the system treats as prior-building.
...
Each AuDHD individual sits somewhere on the gradient between the two locked profiles, with a dominant pull toward one.
...
- **Scout monotropism**: Deep, narrow, temporary. When engaged, intensity can match the systemizer's — volatile prior precision means the channel holds only as long as the PE stream sustains elevated precision on that channel. The temporary prior elevation during intense engagement produces systemizer-like depth, but the elevation is fragile. The moment the PE rate drops, the prior reverts toward its low default and a new channel outbids the current one.
...
If priors are built through architecture-specific channels (§2), they must be revised through those same channels. Each architecture has a **trust channel** — the type of evidence it assigns high precision to when evaluating whether to revise an existing model.
...
| Scout prior precision is volatile, not uniformly low | Refinement of §2.2 | PP-grounded: volatility preserves low-default while explaining temporary elevation | — | NEW v1.6 |
...
*v1.7 — Channel of prior formation as primary differentiator (§2), systemizer model-completeness trajectory (§2.1), architecture-specific trust channels (§4.14), AuDHD gradient-position-dependent hierarchy (§4.5), stress-polarization autonomicity depth (§4.3), trust channel matching prediction (§5.10). Layer separation maintained throughout.*
